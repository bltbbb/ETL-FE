    ...
    methods: {
      ...
      ok(){
        if(this.addChange == 'add'){
          this.$http.post(this.$store.state.domain+'/javaScript',qs.stringify(this.modalData)).then(res=>{
            if(res.data.status == 0){
          ...
          this.$http.put(this.$store.state.domain+'/javaScript',qs.stringify(this.modalData)).then(res=>{
            if(res.data.status == 0){
